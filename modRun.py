...
def main(selected_mod: str, dld_dir: list, tags: bool, tag: list, no_dld: bool):
    ...
    if dld_dir is not None and not no_dld:
        ...
        if "videos" in lk:
            ...
            print(f"* 下载失败: 视频{colors.f_important(lk['errors'])}")
            restore = True
    ...
    if tags:
        ...
        for i, j in mod.lastTags.items():
            print(f"标签编号[{colors.f_important(i)}], 标签名[{colors.f_important(j)}]")
        return 0
    if len(tag) != 0:
        if restore:
    ...
    if mod is not None:
        ...
        for i in range(link_len):
            ...
            if "error" in link:
                lk["errors"].append(i)
            links.append(link)
        print(colors.f_info(f"视频列表获取完成, 将下载视频"))
    else:
    ...
    for i in range(link_len):
        ...
        if no_dld:
            continue
        if i in lk['errors']:
            print(colors.f_warning(f"下载时出错过, 跳过视频[{colors.f_important(i)}]"))
            continue
        if restore and i < lk["progress"]:
            print(colors.f_warning(f"之前已经下载过, 跳过视频[{colors.f_important(i)}]"))
            continue
        print(f"下载视频[{colors.f_important(i)}]...")
